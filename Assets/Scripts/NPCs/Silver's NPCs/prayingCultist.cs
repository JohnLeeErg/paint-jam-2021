...
public class prayingCultist : ExampleNPC
{
    protected override void NodeCreate()
    {
        ...
        //option what
        Node option1 = new Node("Now to begin the ceremonial prayer.", startNode);
        ...
        Node option2 = new Node("BETA CAROTENE, is found in carrots, sweet potatoes and pumpkins and gives the orange color.", startNode);
        ...
        Node option3 = new Node("CANOLA AND SOYBEAN, Vegetable oil.", startNode);
        ...
        Node option4 = new Node("CARAMEL COLOUR, food colour made by heating sugar and adds to the flavour.", startNode);
        ...
        Node option5 = new Node("CARRAGEENAN, used as a thickener to keep our chicken meat juicy.", startNode);
        ...
        Node option6 = new Node("CHICKEN STOCK,made from cooking chicken meat and chicken bones in water and then concentrating.", startNode);
        option5.AddOption("...", option6);
        option5.AddOption("Leave Conversation");


        Node option7 = new Node("CITRIC ACID, used to control the acidity, derived from either sugar beet or corn through fermentation.", startNode);
        option6.AddOption("...", option7);
        option6.AddOption("Leave Conversation");

        Node option8 = new Node("CORN STARCH OR POTATO STARCH, helps to thicken soups and give a consistent texture.", startNode);
        option7.AddOption("...", option8);
        option7.AddOption("Leave Conversation");
        Node option9 = new Node("CORN SYRUP SOLIDS, a sweetener made from corn.", startNode);
        option8.AddOption("...", option9);
        option8.AddOption("Leave Conversation");
        Node option10 = new Node("CREAM, used for its flavour and to give smooth texture.", startNode);
        option9.AddOption("...", option10);
        option9.AddOption("Leave Conversation");
        Node option11 = new Node("DEHYDRATED GARLIC, used to for its flavour.", startNode);
        option10.AddOption("...", option11);
        option10.AddOption("Leave Conversation");
        Node option12 = new Node("DEHYDRATED ONIONS, used as a base flavour.", startNode);
        option11.AddOption("...", option12);
        option11.AddOption("Leave Conversation");
        Node option13 = new Node("DEXTROSE, a sugar found naturally in fruits and honey", startNode);
        option12.AddOption("...", option13);
        option12.AddOption("Leave Conversation");
        Node option14= new Node("EGG NOODLES,  enriched with five essential nutrients and made with flour milled from a mix of durum and spring wheat", startNode);
        option13.AddOption("...", option14);
        option13.AddOption("Leave Conversation");
        Node option15= new Node("GLUTAMIC ACID, an amino acid found in animals and plants such as tomatoes and mushrooms.", startNode);
        option14.AddOption("...", option15);
        option14.AddOption("Leave Conversation");
        option15.AddOption("...", option2);
